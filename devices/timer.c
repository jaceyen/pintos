...
timer_sleep(int64_t ticks)
{
  if(ticks > 0)
  {
    ...
    struct thread *t;
    t = thread_current();
    t->block_ticks = ticks;   //record blocking time
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  enum intr_level old_level; //record original interrupt status
  old_level = intr_disable(); //thread_foreach requires close interruption.
